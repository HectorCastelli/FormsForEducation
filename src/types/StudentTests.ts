class StudentTests {
  ...
  static loadFromSheet(data: any[]): StudentTests {
    ...
    if (JSON.parse(data[1]).name) {
      ...
      result.student = new Student("", data[1]); //TODO: Remove this for formal release, since this is a hack to migrate older sheets entries
    }
  ...
  static findByTestId(testId: string): StudentTests | null {
